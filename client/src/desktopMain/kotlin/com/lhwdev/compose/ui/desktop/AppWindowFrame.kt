...
import com.lhwdev.vrcar.client.App
...
fun TopAppFrame(info: AppRouteInfo) { // this is not good, disabling all the native window controls like snapping to edges
	WindowDraggableArea(
		...
		Row(
			...
			if(
				info.isRoot != true && navigationState.routes.size > 1
			) WindowFrameButton(onClick = { navigationState.popRoute() }) {
			...
			// WindowFrameButton(onClick = { currentComposeWindow.let {
			// 	it.extendedState = it.extendedState or JField
			// } }) {
			// 	Icon(if(isWindowDocked) Icons.Outlined.WindowMaximize
...
fun WindowFrameButton(onClick: () -> Unit, content: @Composable () -> Unit) {
	HoverTextButton(
		...
		colors = ButtonDefaults.textButtonColors(contentColor = Color.White.copy(alpha = 0.03f)),
